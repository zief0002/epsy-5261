...
1. Import the [bachelor.csv](https://raw.githubusercontent.com/zief0002/epsy-5261/main/data/bachelor.csv) data and computing numerical summaries. Based on these summaries, compute *s*, the denominator for Cohen's *d*.
...
2. Compute Cohen's *d* using the values computed in the previous problem..
...
3. Interpret the value of Cohen's *d*. Also comment on its magnitude.
...
4. Use the `smd()` function to compute Cohen's *d* for the cannabis example.
...
5. Use the `ci.smd()` function to compute a 99% CI for Cohen's $\delta$ for the Bachelor/Bachelorette age example. You can change the confidence level using the `conf.level=` argument. The default is `conf.level=.95`, which produces a 95% CI.
